...
var myLocalize = new Localize({
    ...
    'This service is powered by <b>IBM Watson</b> and Skyscanner.': {
        'nl': 'Deze service is powered by <b>IBM Watson</b> en Skyscanner.'
    },
    'Go to <a href=\"http://skyscanner.net\">skyscanner.net</a> to actually book this flight.': {
        'nl': 'Ga naar <a href=\"http://skyscanner.net\">skyscanner.net</a> voor het boeken van deze vlucht.'
    }
});

myLocalize.setLocale(process.env.CONVERSATION_LANG);
            ...
            client.get(poll_url, args, function(data, response) {
              ...
              if (data.Status == 'UpdatesComplete') {
                ...
                if (data.Itineraries[0] === undefined) {
                  ...
                  callback(null, {text: [myLocalize.translate('We\'ve found a flight for you for ') + '<b>€' + Math.round(price) + '</b>.',
                                        ' ',
                                        myLocalize.translate('Outbound flight: ') + outboundCarrierCode + outboundFlightNumber + myLocalize.translate(' on ') + outboundDepature.substring(0,10) + ', ' + params.origin + ' ' + outboundDepature.substring(11,16) + ' -> ' + params.destination + ' ' + outboundArrival.substring(11,16),
                                        myLocalize.translate('Operated by: ') + '<i>' + outboundOperatorCarrierName + '</i>',
                                        ' ',
                                        myLocalize.translate('Inbound flight: ') + inboundCarrierCode + inboundFlightNumber + myLocalize.translate(' on ') + inboundDepature.substring(0,10) + ', ' + params.destination + ' ' + inboundDepature.substring(11,16) + ' -> '+ params.origin + ' ' + inboundArrival.substring(11,16),
                                        myLocalize.translate('Operated by: ') + '<i>' + inboundOperatorCarrierName + '</i>',
                                        ' ',
                                        myLocalize.translate('This service is powered by <b>IBM Watson</b> and Skyscanner.'),
                                        myLocalize.translate('Go to <a href=\"http://skyscanner.net\">skyscanner.net</a> to actually book this flight.')
                  ]});
                }
              }
            })
